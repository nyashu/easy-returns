...
@section('content')
                        ...
                        <table class="table align-items-center table-flush">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th scope="col">Return Frequency</th>
                            ...
                            <tbody>
                                @forelse ($stores as $store)
                                    <tr>
                                        ...
                                        <th><span class="text-danger">{{ $store->return_frequency }}</span></th>
                                        <th><span class="text-success">{{ $store->total_store_return }}</span></th>
                                        <th>
                                            <div class="d-flex justify-content-between">
                                                <i class="fa-solid cursor fa-eye text-primary"  data-toggle="modal"
                                                data-target="#showStoreModal-{{ $store->id }}"></i>
                                                <i class="fa-solid cursor fa-pen-to-square"></i>
                                                <i class="fa-sharp fa-solid cursor fa-trash text-danger" data-toggle="modal"
                                                    data-target="#deleteStoreModal" data-store-id="{{ $store->id }}"></i>
                                            </div>

                                            <!--Show Modal -->
                                            @foreach ($stores as $store)
                                                <div class="modal fade" id="showStoreModal-{{ $store->id }}"
                                                    tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                                                    aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                                        <div class="modal-content">
                                                            <div class="modal-header text-center">
                                                                <h5 class="modal-title" id="exampleModalLabel">Store
                                                                    Information</h5>
    ...
    <div class="modal fade" id="deleteStoreModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form method="POST" id="deleteStoreForm">
                    @method('DELETE')
                    @csrf()
                    ...
                    <div class="modal-body">
                        You are trying to remove the store. Are you sure ?
                    </div>
...
@push('js')
    <script>
        $('#deleteStoreModal').on('show.bs.modal', function(event) {
            ...
            $('#deleteStoreForm').attr('action', '/stores/' + button.data('store-id'));
        });
    </script>
@endpush
